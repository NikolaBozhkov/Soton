...
public class Tester {
	...
	public static void testPart3() {
		...
		// Create a new door and sets it's name
		Door door = new Door();
		door.setRoomName("Research Labs");
		
		// Create a new lock and attach it to the door
		CardLock lock = new CardLock();
		door.attachLock(lock);
		
		// Create a new student's card and swipe it
		SmartCard card = new SmartCard("Bob");
		lock.swipeCard(card);
		
		// Test if the door can be opened
		System.out.println("Opening door with student card(Should not open)...\n");
		door.openDoor();
		
		// Set the card to staff and swipe it again
		card.setStaff(true);
		lock.swipeCard(card);
		
		// Test if the door can open with staff's card
		System.out.println("\nOpening door with staff card(Should open)...\n");
		door.openDoor();
		
		// Test case where door is opened without a lock being attached
		System.out.println("\nTrying to open a door without a lock\n");
		Door doorNoLock = new Door();
		doorNoLock.setRoomName("Under Construction");
		doorNoLock.openDoor();
	}
}